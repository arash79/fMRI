...
class BASC:
    ...
    SUPPORTED_RESOLUTIONS = [7, 12, 20, 36, 64, 122, 197, 325, 444]

    def __init__(self, data_paths, resolution):

        assert resolution in self.SUPPORTED_RESOLUTIONS, f"Resolution {resolution} is not supported."

        self.data_paths = data_paths
        self.resolution = resolution
        self.atlas = datasets.fetch_atlas_basc_multiscale_2015(resolution=resolution)
    
        self.data = self.__load_data()

    def __load_data(self):
        return {file_path.split('\\')[2]: image.load_img(file_path) for file_path in self.data_paths}
    
    def parcellate(self):
        
        labels = [f'network_{i:03d}' for i in range(self.resolution)]
    ...
    def plot_parcellations_using_matplotlib(self, view='lateral'):
        ...
        title = f'BASC Atlas - Parcellated Regions'
    ...
    def plot_parcellations_using_plotly(self):
        ...
        title = 'BASC Atlas - Parcellated Regions'
